...
class JSONMetricsResource(Resource):
    ...
    def get_exemplar(sample, metric):
        if not sample.exemplar:
            ...
            raise ValueError(
                f"Metric {metric.name} has exemplars, but is not a histogram bucket"
            )
        return {
    ...
    def generate_latest_json(self):
        ...
        return json.dumps(output, cls=MetricsEncoder).encode('utf-8')
    ...
    collectors: List[MetricsCollector] = [
        CommitmentMadeEventMetricsCollector(
            event_args_config={
                ...
                "period": (
                    Gauge,
                    f'{metrics_prefix}_activity_confirmed_period',
        ...
        EventMetricsCollector(
            ...
            event_args_config={
                "value": (
                    Gauge,
                    f'{metrics_prefix}_mined_value',
                    'Minted value',
                ),
                "period": (
                    Gauge,
                    f'{metrics_prefix}_mined_period',
                    'Minted period',
                ),
                "block_number": (
                    Gauge,
                    f'{metrics_prefix}_mined_block_number',
                    'Minted block number',
                ),
            },
        ...
        EventMetricsCollector(
            ...
            event_args_config={
                "penalty": (
                    Gauge,
                    f'{metrics_prefix}_last_slashed_penalty',
        ...
        OperatorBondedEventMetricsCollector(
            event_args_config={
                "startPeriod": (
                    Gauge,
                    f'{metrics_prefix}_worker_set_start_period',
                    'New worker was bonded',
                ),
                "block_number": (
                    Gauge,
                    f'{metrics_prefix}_worker_set_block_number',
                    'OperatorBonded block number',
                ),
            },
            staker_address=staker_address,
            operator_address=ursula.operator_address,
