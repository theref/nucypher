...
def public_keys(general_config, character_options, config_file):
    ...
    return BOB.controller.public_keys()
...
def retrieve_and_decrypt(general_config,
    ...
    if not alice_verifying_key:
        if alice:  # from storage
            ...
            emitter.message(
                f'{card.nickname or f"Alice #{card.id.hex()}"}\nVerifying Key  | {bytes(card.verifying_key).hex()}',
                color='green',
            )
            if not force:
    ...
    if decode:
        messages = [b64decode(r).decode() for r in response['cleartexts']]
