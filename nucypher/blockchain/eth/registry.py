...
class GithubRegistrySource(CanonicalRegistrySource):
    ...
    def get_publication_endpoint(self) -> str:
        return f'{self._BASE_URL}/main/nucypher/blockchain/eth/contract_registry/{self.network}/{self.registry_name}'
...
class EmbeddedRegistrySource(CanonicalRegistrySource):
    ...
    def get_publication_endpoint(self) -> Path:
        return Path(
            CONTRACT_REGISTRY_BASE / self.network / self.registry_name
        ).absolute()
...
class BaseContractRegistry(ABC):
    ...
    def __eq__(self, other) -> bool:
        return True if self is other else self.id == other.id

    def __repr__(self) -> str:
        return f"{self.__class__.__name__}(id={self.id[:6]})"
    ...
    def enrolled_names(self) -> Iterator:
        return iter(record[0] for record in self.read())

    @property
    def enrolled_addresses(self) -> Iterator:
        return iter(record[2] for record in self.read())
    ...
    def enroll(self, contract_name, contract_address, contract_abi, contract_version) -> None:
        ...
        try:
            ...
            self.log.info(
                f"Blank registry encountered: enrolling {contract_name}:{contract_version}:{contract_address}"
            )
            registry_data = []
        ...
        self.log.info(
            f"Enrolled {contract_name}:{contract_version}:{contract_address} into registry."
        )
    ...
    def search(self, contract_name: str = None, contract_version: str = None, contract_address: str = None) -> tuple:
        ...
        return tuple(contracts) if contract_name else contracts[0]
...
class LocalContractRegistry(BaseContractRegistry):
    ...
    def read(self) -> Union[list, dict]:
        ...
        try:
            with open(self.filepath, 'r') as registry_file:
                self.log.debug(f"Reading from registry: filepath {self.filepath}")
                registry_file.seek(0)
                if file_data := registry_file.read():
                    try:
                        registry_data = json.loads(file_data)
                    except JSONDecodeError:
                        raise self.RegistryError(f"Registry contains invalid JSON at '{self.__filepath}'")
                else:
                    registry_data = [] if self._multi_contract else {}

        except FileNotFoundError:
            raise self.NoRegistry(f"No registry at filepath: {self.filepath}")
    ...
    def from_dict(cls, payload: dict, **overrides) -> 'LocalContractRegistry':
        ...
        return cls(filepath=payload['filepath'])
...
class TemporaryContractRegistry(LocalContractRegistry):
    ...
    def clear(self):
        self.log.info(f"Cleared temporary registry at {self.filepath}")
    ...
    def commit(self, filepath) -> Path:
        ...
        self.log.info(f"Committing temporary registry to {filepath}")
        self._swap_registry(filepath)                     # I'll allow it

        if filepath.exists():
            self.log.debug(f"Removing registry {filepath}")
        ...
        self.log.info(f"Wrote temporary registry to filesystem {filepath}")
        return filepath
...
class InMemoryContractRegistry(BaseContractRegistry):
    ...
    def commit(self, filepath: Optional[Path] = None, overwrite: bool = False) -> Path:
        ...
        self.log.info(f"Wrote in-memory registry to '{filepath}'")
        return filepath

    def _destroy(self) -> None:
        self.__registry_data = {}
